"""Preview or import a batch of legacy growth SQLite databases."""
...
from plate_reader.infrastructure.importers import import_legacy_growth_file

DEFAULT_ACTOR_ID = "legacy-migration-editor"
DEFAULT_ACTOR_EMAIL = "legacy-migration@example.invalid"
...
def main() -> None:
    ...
    if not sources:
        parser.error("No legacy SQLite files were found")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=Path,
        help="Legacy SQLite files or directories (non-recursive)",
    )
    parser.add_argument(
        "--backend",
    ...
    parser.add_argument(
        "--commit",
        action="store_true",
        help="Write verified imports; omission is always a no-write dry run",
    )
    ...
    parser.add_argument(
        "--bootstrap-editor",
        action="store_true",
        help="Explicitly create/update the named migration editor before a commit",
    )
        ...
        repository.upsert_user(
            {
                ...
                "email": actor.email,
                "display_name": "Legacy Migration Editor",
                "role": actor.role,
                "is_active": True,
            }
        )
